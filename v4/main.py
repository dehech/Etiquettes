    ...
    try:
        ...
        with pdfplumber.open(fichier) as pdf:
            for page in pdf.pages:
                t = page.extract_text()
                if t:
                    lignes.extend(t.splitlines())

        # --- 3 articles max par ligne ---
        temp_articles = [{}, {}, {}]
        ...
        for line in lignes:
            ...
            # Supprimer le label
            values_part = line.replace(label, "").strip()

            # Découper les colonnes (PDF = espaces multiples)
            values = re.split(r"\s{2,}", values_part)

            # Stocker dans les 3 articles temporaires
            for i, val in enumerate(values):
                if i < 3:
                    temp_articles[i][key] = val.strip()

            # Vérifier si un article est complet
            for article in temp_articles:
                if "ref" in article and "type" in article and "prix" in article:
                    liste.append((
                        article.get("ref", ""),
                        article.get("type", ""),
                        article.get("prix", ""),
        ...
        messagebox.showinfo(
            "Import réussi",
            f"{len(liste)} articles importés correctement depuis le PDF."
        )

    except Exception as e:
        messagebox.showerror("Erreur", f"Impossible d'importer le PDF\n{e}")
...
labels = ["Référence", "Type", "Prix TTC", "Description"]
entries = []

for i, label in enumerate(labels):
    tk.Label(frame, text=label + " :", font=FONT_BOLD, bg=TEXT_BG).grid(
        row=i, column=0, sticky="w", pady=6
    )
    e = tk.Entry(frame, font=FONT, width=35)
    e.grid(row=i, column=1, pady=6, padx=(5, 0))
    entries.append(e)

entry_ref, entry_type, entry_prix, entry_desc = entries

# Taille police PDF
tk.Label(frame, text="Taille écriture PDF :", font=FONT_BOLD, bg=TEXT_BG).grid(
...
spin_font_size.grid(row=4, column=1, sticky="w", pady=(10, 0))
...
btn_ajouter.grid(row=0, column=0, padx=5)

tk.Button(
    btn_frame, text=" Générer PDF ",
    command=imprimer,
    bg="#4CAF50", fg="white",
    font=FONT_BOLD,
    width=18
...
tk.Button(
    btn_frame, text=" Importer PDF ",
    command=importer_pdf,
    bg="#FF9800", fg="white",
    font=FONT_BOLD,
    width=18
).grid(row=0, column=2, padx=5)

btn_remise_zero = tk.Button(
    btn_frame, text=" Remise à zéro ",
    command=remise_a_zero,
    bg="#9E9E9E", fg="white",
    font=FONT_BOLD,
    width=18
)
btn_remise_zero.grid(row=0, column=3, padx=5)

btn_supprimer = tk.Button(
    btn_frame, text=" Supprimer ",
    command=supprimer_article,
    bg=DANGER, fg="white",
    font=FONT_BOLD,
    width=18
)
...
list_frame.grid(row=3, column=0, columnspan=4, padx=15, pady=10)

text_list_display = scrolledtext.ScrolledText(
    list_frame,
    width=70,
    height=12,
    font=FONT,
...
text_list_display.pack(fill=tk.BOTH, expand=True)
